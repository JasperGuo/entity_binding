...
def prepare_tables(file, word_vocab_file, char_vocab_file, data_type_file):
    ...
    with open(".\\active_learning\\tables.txt", "w") as f:
        f.write('\n'.join(result))
...
def prepare_question(file, word_vocab_file, char_vocab_file):
    ...
    # print(len(result))
    with open(".\\active_learning\\questions.txt", "w") as f:
        f.write('\n'.join(result))


if __name__ == "__main__":
    prepare_tables("..\\data\\active_learning\\tables.txt", ".\\vocab\\word_dict.json", ".\\vocab\\char_dict.json", ".\\vocab\\data_type.json")
    prepare_question("..\\data\\active_learning\\questions.txt", ".\\vocab\\word_dict.json", ".\\vocab\\char_dict.json")
